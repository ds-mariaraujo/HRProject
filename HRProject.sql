...
SELECT emp.dep_id, 
	dep.dep_name, 
	CONVERT(DECIMAL(8,2),AVG(CAST(salary AS DECIMAL(9,2)))) AS avg_salary_by_departament
FROM employee emp
...
Executive Office while the lowest average salary ($ 59953.55) is in the Production department 
*/
...
/*
The lowest salary of the company belongs to Colleen Zima, which is $45,046.00
and his position is Production Technician I
...
SELECT CONVERT(DECIMAL(5,2),AVG(CONVERT(DECIMAL(5,2), DATEDIFF(year, dob, CAST(GETDATE() AS DATE))))) AS average_age
FROM employee emp
...
/*
The average age of all current employees is 42.77 years
*/

-- Average time worked on the company for the Employees who left

SELECT CONCAT(CONVERT(DECIMAL(5,2),AVG(CAST(DATEDIFF(month, emp.date_hire, emp.date_termination) AS DECIMAL(6,2)))), ' ', 'months') AS average_time
FROM employee emp
	JOIN employment_status sta
	ON emp.empl_status_id = sta.empl_status_id
WHERE sta.empl_name <> 'Active'
...
/*
For the employees who already left, 41.49 months was the worked average time in the company 
*/
...
WITH salary_amount AS (
	SELECT emp_id, date_hire, date_termination,
	CASE
		WHEN date_termination IS NULL THEN ((CAST(DATEDIFF(MONTH, date_hire, GETDATE()) AS DECIMAL(5,2)))/12) * salary
		ELSE ((CAST(DATEDIFF(MONTH, date_hire, date_termination) AS DECIMAL(5,2)))/12) * salary
	END AS amount
	FROM employee 
)
...
/*
The total spent by the company, until the current date, on employees' salaries is $152,301,893.31
*/
